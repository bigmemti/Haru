        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('dashboard.category.store') }}" method="POST" x-data="{name:'{{ old('name') }}'}" enctype="multipart/form-data">
                        @csrf
                        <div class="flex flex-col gap-8">
                            ...
                            <label class="input input-bordered flex items-center gap-2 ">
                                {{ __('Slug') }}
                                <input type="text" class=" border-0 focus:ring-0" :value='name.trim().replace(/\s+/g, "-")' placeholder="{{ Str::slug('کره بادوم زمینی', '-', null) }}"  disabled/>
                            </label>

                            <select class="select select-bordered" name="parent_id" @disabled(!$categories->count())>
                                <option value="" @selected(!old('parent_id'))>{{ __('Parent Category') }}</option>
